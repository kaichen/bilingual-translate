...
// 最大并发数由边缘注入（configureQueue），默认 6
let maxConcurrentProvider: () => number = () => 6;

// 由使用方（translateApi）启动时注入实时读取 config 的 provider，使本模块无需 import config
export function configureQueue(getMaxConcurrent: () => number): void {
  maxConcurrentProvider = getMaxConcurrent;
}

// 获取最大并发翻译数量
function getMaxConcurrentTranslations(): number {
  return maxConcurrentProvider() || 6; // 默认值为6
}
  ...
  return new Promise((resolve, reject) => {
    ...
    const taskWrapper = async () => {
      ...
      try {
        ...
        // reject 已通知外层 Promise；不再 rethrow —— 直接调用路径 taskWrapper() 未 catch，throw 会产生未处理的 rejection
        reject(error);
      } finally {
