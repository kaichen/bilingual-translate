...
// @namespace    https://bilingual-translate.local/
// @version      1.31
...
// cacheKey 与 时间
const checkKey = "bilingual_translate_check";
...
// trustHTML
let safeBilingualTranslate;
if (window.trustedTypes && window.trustedTypes.createPolicy) {
    safeBilingualTranslate = window.trustedTypes.createPolicy("safeBilingualTranslate", {
...
const settingManager = {
  ...
  <div style="font-size: 1em;" xmlns="http://www.w3.org/1999/html">
    <label class="instant-setting-label">快捷键<select id="bilingual-translate-hotkey" class="instant-setting-common">${this.generateOptions(shortcutManager.hotkeyOptions, util.getValue('hotkey'))}</select></label>
    <label class="instant-setting-label">翻译源语言<select id="bilingual-translate-from" class="instant-setting-common">${this.generateOptions(langManager.from, langManager.getFrom())}</select></label>
    <label class="instant-setting-label">翻译目标语言<select id="bilingual-translate-to" class="instant-setting-common">${this.generateOptions(langManager.to, langManager.getTo())}</select></label>
    <label class="instant-setting-label">翻译服务<select id="bilingual-translate-model" class="instant-setting-select">${this.generateOptions(transModelName, util.getValue('model'))}</select></label>
    
    <!--支持 ollama 等自定义模型名称-->
    <label class="instant-setting-label" id="bilingual-translate-custom-type-label" style="display: none;">
    <span class="bilingual-translate-tooltip">自定义模型类型
            <span class="bilingual-translate-tooltiptext">
            请填写模型类型全称，如：gemma:7b、llama2:7b
            </span>
        </span>
    <input type="text" class="instant-setting-input" id="bilingual-translate-custom-type" value="${optionsManager.getOption(util.getValue('model'))}" ></label>
    
    <label class="instant-setting-label" id="bilingual-translate-option-label" style="display: none;">模型类型<select id="bilingual-translate-option" class="instant-setting-select"></select></label> 
    <!-- custom 输入框-->
    <label class="instant-setting-label" id="bilingual-translate-custom-label" style="display: none;">
        <span class="bilingual-translate-tooltip">自定义 GPT 地址
            <span class="bilingual-translate-tooltiptext">
        ...
        <input type="text" class="instant-setting-input" id="bilingual-translate-custom" value="${customGPT.getGPTUrl()}" >
    </label>
    <!-- 令牌区域 -->
    <label class="instant-setting-label" id="bilingual-translate-token-label" style="display: none;">token令牌<input type="text" class="instant-setting-input" id="bilingual-translate-token" value="" ></label>
    <label class="instant-setting-label" id="bilingual-translate-ak-label" style="display: none;">ak令牌<input type="text" class="instant-setting-input" id="bilingual-translate-ak" value="" ></label>
    <label class="instant-setting-label" id="bilingual-translate-sk-label" style="display: none;">sk令牌<input type="text" class="instant-setting-input" id="bilingual-translate-sk" value="" ></label>
    <!-- 添加的输入区域 -->
    <label class="instant-setting-label" id="bilingual-translate-system-label" style="display: none;">
        <span class="bilingual-translate-tooltip">system角色设定<span class="bilingual-translate-tooltiptext">模型角色设定，如：你是一名专业的翻译家</span></span>
        <textarea class="instant-setting-textarea" id="bilingual-translate-system-message">${chatMgs.getSystemMsg()}</textarea>
    </label>
    <label class="instant-setting-label" id="bilingual-translate-user-label" style="display: none;">
        <span class="bilingual-translate-tooltip">user消息模板<span class="bilingual-translate-tooltiptext">用户对话内容，如：请你翻译 Hello</br>注意：{{text}} 是你需要翻译的原文，不可缺少。</span></span>
        <textarea class="instant-setting-textarea" id="bilingual-translate-user-message">${chatMgs.getOriginUserMsg()}</textarea>
    </label>
  </div>`;
        Swal.fire({
                title: '设置中心',
                ...
                if (result.isConfirmed) {
                    let model = util.getElementValue('bilingual-translate-model');

                    // 0、设置自定义 GPT 地址
                    if ([transModel.openai, transModel.ollama].includes(model)) {
                        let ok = customGPT.setGPTUrl(model, util.getElementValue('bilingual-translate-custom'));
                        if (!ok) {
                            toast.fire({
                                icon: 'error',
                    ...
                    // 1、设置语言
                    util.setValue('from', util.getElementValue('bilingual-translate-from'));
                    util.setValue('to', util.getElementValue('bilingual-translate-to'));
                    // 2、设置快捷键
                    util.setValue('hotkey', util.getElementValue('bilingual-translate-hotkey'));
                    // 3、设置翻译服务
                    util.setValue('model', model);
                    // 4、设置模型类型
                    if (model === transModel.ollama) {
                        optionsManager.setOption(model, util.getElementValue('bilingual-translate-custom-type'));
                    } else {
                        optionsManager.setOption(model, util.getElementValue('bilingual-translate-option'));
                    }
                    // 5、存储 token
                    let token = util.getElementValue('bilingual-translate-token');
                    let ak = util.getElementValue('bilingual-translate-ak');
                    let sk = util.getElementValue('bilingual-translate-sk');
                    switch (model) {
                    ...
                    // 6、设置 chatGPT 消息模板
                    chatMgs.setSystemMsg(util.getElementValue('bilingual-translate-system-message'));
                    chatMgs.setUserMsg(util.getElementValue('bilingual-translate-user-message'));
        ...
        // 监听“翻译服务”选择框
        document.getElementById('bilingual-translate-model').addEventListener('change', e => {
    ...
    showHidden(model) {
        // label 是最外层的标签
        const tokenLabel = document.getElementById('bilingual-translate-token-label');
        const akLabel = document.getElementById('bilingual-translate-ak-label');
        const skLabel = document.getElementById('bilingual-translate-sk-label');

        const token = document.getElementById('bilingual-translate-token');
        const ak = document.getElementById('bilingual-translate-ak');
        const sk = document.getElementById('bilingual-translate-sk');
    ...
    setDisplayStyle(flex, none) {
        // 消息模版
        const systemMsgLabel = document.getElementById('bilingual-translate-system-label');
        const userMsgLabel = document.getElementById('bilingual-translate-user-label');
        // 自定义 GPT 地址框
        const customLabel = document.getElementById('bilingual-translate-custom-label');
        const optionLabel = document.getElementById('bilingual-translate-option-label');
        const customTypeLabel = document.getElementById('bilingual-translate-custom-type-label');
        ...
        document.getElementById('bilingual-translate-custom').value = customGPT.getGPTUrl(model);  // 设置自定义 GPT 地址
        ...
        if (model === transModel.ollama) {
            ...
            document.getElementById('bilingual-translate-custom-type').value = optionsManager.getCustomOption(model);
        } else {
            const optionSelect = document.getElementById('bilingual-translate-option');
        ...
        if (outerHTMLCache) {
            ...
            setTimeout(() => {  // 延迟 remove 转圈动画与替换文本
                ...
                if (fn) {
                    ...
                    node.outerHTML = safeBilingualTranslate ? safeBilingualTranslate.createHTML(outerHTMLCache) : outerHTMLCache;
                }
        ...
        transModelFn[model](origin).then(text => {
            ...
            if (isMachineTrans(model)) {
                ...
                if (fn) {
                    ...
                    node.outerHTML = safeBilingualTranslate ? safeBilingualTranslate.createHTML(text) : text;
                }
            } else {
                // LLM 翻译
                node.innerHTML = safeBilingualTranslate ? safeBilingualTranslate.createHTML(text) : text;
                newOuterHtml = node.outerHTML;
            }
...
function createLoadingSpinner(node, isCache) {
    ...
    spinner.className = 'loading-spinner-bilingualtranslate';
...
function hasLoadingSpinner(node) {
    if (node.classList.contains('loading-spinner-bilingualtranslate')) return true;
...
function initApplication() {
    ...
    .loading-spinner-bilingualtranslate {border: 3px solid #f3f3f3;border-top: 3px solid blue;border-radius: 50%;width: 12px;height: 12px;animation: spin 1s linear infinite;display: inline-block;}
    ...
    /* 工具提示 */
    .bilingual-translate-tooltip { position: relative; display: inline-block; }
    .bilingual-translate-tooltip .bilingual-translate-tooltiptext { visibility: hidden; width: 25em; background-color: black; color: #fff; text-align: left; border-radius: 6px; padding: 5px; position: absolute; z-index: 1; bottom: 100%; left: 50%; margin-left: -60px; opacity: 0; transition: opacity 0.6s; font-size: 14px; }
    .bilingual-translate-tooltip:hover .bilingual-translate-tooltiptext { visibility: visible; opacity: 1; }
    ...
    @media (max-width: 600px) {
        ...
        .bilingual-translate-tooltip .bilingual-translate-tooltiptext {
            width: 18em; /* 调整工具提示宽度 */
    ...
    if (isEmpty(lastRun) || now - lastRun > expiringTime) {
        ...
        GM_xmlhttpRequest({
            ...
            onload: function (response) {
                ...
                // 将 bilingual_translate_check 设置为新的缓存
