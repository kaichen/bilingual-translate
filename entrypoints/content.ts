...
let translationStatusRoot: Root | null = null;
...
export default defineContentScript({
    ...
    async main() {
        ...
        document.addEventListener('bilingualtranslate-toggle-translation', () => {
...
function setupFloatingBallHotkey() {
    ...
    if (isDev) {
        console.log(`[bilingual translate] 设置悬浮球快捷键: ${config.floatingBallHotkey}, 系统: ${isMac ? 'macOS' : '其他'}`);
    }
    ...
    document.addEventListener('keydown', (event) => {
        ...
        if (exactMatch) {
            ...
            // 通过自定义事件来触发翻译
            document.dispatchEvent(new CustomEvent('bilingualtranslate-toggle-translation'));
            
            if (isDev) {
                const activeHotkey = config.floatingBallHotkey === 'custom' 
                    ? config.customFloatingBallHotkey 
                    : config.floatingBallHotkey;
                console.log(`[bilingual translate] 触发悬浮球翻译，快捷键: ${activeHotkey}`);
            }
        }
    });
...
function clearAllTranslations() {
    // 1. 移除所有翻译结果元素
    document.querySelectorAll('.bilingual-translate-translation').forEach(el => el.remove());

    // 2. 移除所有加载状态
    document.querySelectorAll('.bilingual-translate-loading').forEach(el => el.remove());

    // 3. 移除所有错误状态
    document.querySelectorAll('.bilingual-translate-failure').forEach(el => el.remove());

    // 4. 移除所有翻译相关的类名
    document.querySelectorAll('.bilingual-translate-processed').forEach(el => {
        el.classList.remove('bilingual-translate-processed');
    });
...
function mountTranslationStatusComponent() {
    if (translationStatusRoot) return;

    // 创建容器元素
    const container = document.createElement('div');
    container.id = 'bilingual-translate-translation-status-container';
    document.body.appendChild(container);
    
    translationStatusRoot = createRoot(container);
    translationStatusRoot.render(createElement(TranslationStatus));
}
...
function createTranslationTooltip(element: HTMLElement, message: string, type: 'translating' | 'success' | 'error'): HTMLElement {
    ...
    tooltip.className = `bilingual-input-tooltip ${type}`;
    tooltip.id = 'bilingual-input-translation-tooltip';
...
function removeExistingTooltip(): void {
    const existing = document.getElementById('bilingual-input-translation-tooltip');
    if (existing) {
        if (!config.animations) {
            // 如果禁用动画，直接移除
            existing.remove();
...
function addInputBoxAnimation(element: HTMLElement, animationType: 'translating' | 'success' | 'error'): void {
    ...
    // 移除已存在的动画类
    element.classList.remove('bilingual-input-translating', 'bilingual-input-success', 'bilingual-input-error');
    
    // 添加新的动画类
    element.classList.add(`bilingual-input-${animationType}`);
    
    // 如果不是翻译中的动画，在动画完成后移除类
    if (animationType !== 'translating') {
        setTimeout(() => {
            element.classList.remove(`bilingual-input-${animationType}`);
    ...
    try {
        ...
        try {
            ...
            if (translatedText && translatedText !== cleanedText) {
                // 移除翻译中的动画
                element.classList.remove('bilingual-input-translating');
