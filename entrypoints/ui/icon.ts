...
import { services, options } from "../config/option";
...
export function insertFailedTip(
  ...
  wrapper.classList.add("bilingual-translate-retry-wrapper");

  // 创建重试按钮
  const retryBtn = document.createElement("span");
  retryBtn.innerText = "重试";
  retryBtn.classList.add("bilingual-translate-retry");
  retryBtn.addEventListener("click", handleRetryClick(node, wrapper));

  // 添加失败标记
  node.classList.add("bilingual-translate-failure");

  // 创建错误信息提示按钮
  const errorTip = document.createElement("span");
  errorTip.innerText = "错误原因";
  errorTip.classList.add("bilingual-translate-reason");
...
function handleRetryClick(node: HTMLElement, wrapper: HTMLElement) {
  return (event: MouseEvent) => {
    ...
    node.classList.remove("bilingual-translate-failure"); // 移除失败标记
