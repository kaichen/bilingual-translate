...
An open-source bilingual browser translation extension that provides everyone with a native-like reading experience.
...
The extension has not been published to browser stores yet. Build from source and load it manually:
...
The build output is `.output/chrome-mv3`, which can be loaded from the browser extension management page in developer mode.

## 📖 Documentation

See the local `docs/` directory for:
